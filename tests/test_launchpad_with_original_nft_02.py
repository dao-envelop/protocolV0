...
def test_distr(accounts,  distributor, niftsy20, dai, launcpad, ERC721Distr, weth):
    ...
    with reverts("Ownable: caller is not the owner"):
        launcpad.setEnableAfterDate(UNWRAP_AFTER, {"from": accounts[1]})
    launcpad.setEnableAfterDate(UNWRAP_AFTER, {"from": accounts[0]})
    with reverts("Please wait for start date"):
        launcpad.claimNFT(1, dai)

    with reverts("Ownable: caller is not the owner"):
        launcpad.setPrice(dai, 3, 100, {"from": accounts[1]})